...
import { faChevronUp, faChevronDown, faLocationDot, faLink } from '@fortawesome/free-solid-svg-icons'
...
const ExperienceCard = ({xp}: {xp: Experience}) => {
  ...
  return (
    <div id={xp.name} className={styles.card + (isExpanded ? ' ' + styles.expanded : '')}>
      ...
      <section {...getCollapseProps()}>
        <div className='pure-g'>
          <div className={styles.logo + ' pure-u-1-5'}>
            <Image src={`/images/xp/${xp.name.replace(/\d/, '')}.svg`}
              height={50} width={100} alt = {xp.name.replace(/\d/, '') + " logo"}/>
              <p className='small'>
                <a href={xp.maps} target='_blank' rel="noreferrer">
                  <FontAwesomeIcon icon={faLocationDot} />
                   {xp.location}
                </a><br/>
                <a href={t(`${xp.name}.website`)} target='_blank' rel="noreferrer">
          ...
          <div className='pure-u-4-5'>
            <div className={styles.content}>
              ...
              <ul className={styles.technos}>
                {xp.technos.map(techno => <li key={techno}>{techno}</li>)}
              </ul>
            </div>
          </div>
        </div>
